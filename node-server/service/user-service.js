...
class UserService {
  ...
  async searchUser(username) {
    let user = "%" + username.split("").join("%") + "%";
    // console.log(user);
    const statement = `
      SELECT u.id as id, u.username as username, u.profilePicture as profilePicture 
      from users as u WHERE username LIKE ?;
    `;

    try {
      const [result] = await connections.execute(statement, [user]);
